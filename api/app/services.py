...
# ---------------------------------------------------------------------------
from staypulse.signals import calendar as _cal  # noqa: E402

_HOLIDAY_EVAL = Path(__file__).resolve().parents[2] / "reports" / "holiday_forecast_eval.json"


def rm_holiday_effect() -> dict[str, Any]:
    return _cal.summary()


def rm_holiday_validation() -> dict[str, Any]:
    return _cal.validate_against_planted()


def rm_holiday_forecast_evaluation() -> dict[str, Any]:
    """Served from the stored evaluation - the backtest is far too slow per request."""
    if not _HOLIDAY_EVAL.exists():
        raise RuntimeError("Holiday forecast evaluation has not been generated yet.")
    stored = json.loads(_HOLIDAY_EVAL.read_text(encoding="utf-8"))
    stored["source"] = "stored evaluation, regenerated by scripts/run_calendar_analysis.py"
    return stored
...
# Decision replay.
#
# A single replay costs roughly five seconds: a dozen queries, several of them
# over the demand grain. That is slow for a page and acceptable for an
# investigative endpoint nobody polls, so it runs live rather than being served
# from a snapshot -- an as-of date is a free parameter and a stored answer could
# only cover the dates someone chose in advance.
#
# The pooled evaluation across many origins IS stored, for the same reason the
# forecast backtest is: fourteen replays is a minute of work.
# ---------------------------------------------------------------------------
from staypulse.analytics import replay as _rp  # noqa: E402

_REPLAY_EVAL = Path(__file__).resolve().parents[2] / "reports" / "replay_evaluation.json"


def rm_replay(as_of: dt.date, horizon_days: int, with_outcome: bool) -> dict[str, Any]:
    return _rp.replay(as_of, horizon_days=horizon_days, with_outcome=with_outcome)


def rm_replay_summary(as_of: dt.date, horizon_days: int) -> dict[str, Any]:
    return _rp.summary(as_of, horizon_days=horizon_days)


def rm_replay_evaluation() -> dict[str, Any]:
    """Pooled replay accuracy across many origins, from the stored evaluation."""
    if not _REPLAY_EVAL.exists():
        raise RuntimeError("Replay evaluation has not been generated yet.")
    stored = json.loads(_REPLAY_EVAL.read_text(encoding="utf-8"))
    stored["source"] = "stored evaluation, regenerated by scripts/run_replay_analysis.py"
    return stored
...
#
# Both are served from the stored evaluation. A 365-day backtest is ~20 seconds
# and the coverage walk rebuilds an interval at every origin on top of that --
# firmly a scheduled job, not a request. The forward forecast WITH intervals is
# computed live, because it is a single origin and the calibration window is
# bounded.
# ---------------------------------------------------------------------------
from staypulse.analytics import intervals as _iv  # noqa: E402

_INTERVAL_EVAL = Path(__file__).resolve().parents[2] / "reports" / "forecast_intervals.json"


def rm_forecast_intervals(horizon_days: int, model: str, level: float) -> dict[str, Any]:
    return _iv.forward(horizon=horizon_days, model=model, level=level)


def rm_interval_coverage() -> dict[str, Any]:
    """Out-of-sample coverage for every method and level, from the stored study."""
    if not _INTERVAL_EVAL.exists():
        raise RuntimeError("Interval evaluation has not been generated yet.")
    stored = json.loads(_INTERVAL_EVAL.read_text(encoding="utf-8"))
    return {
        "generated_at": stored["generated_at"],
        "target": stored["target"],
        "window": stored["window"],
        "default_method": stored["default_method"],
        "coverage": stored["coverage"],
        "source": "stored evaluation, regenerated by scripts/run_interval_analysis.py",
        "note": (
            "Out-of-sample coverage rebuilds the interval at every evaluation "
            "origin from residuals whose target had already been realised there. "
            "The in-sample column is published beside it as a contrast: an "
            "empirical quantile reproduces its own nominal level by construction."
        ),
    }


def rm_backtest_lab() -> dict[str, Any]:
    """Accuracy sliced by horizon, month, weekday and holiday adjacency."""
    if not _INTERVAL_EVAL.exists():
        raise RuntimeError("Interval evaluation has not been generated yet.")
    stored = json.loads(_INTERVAL_EVAL.read_text(encoding="utf-8"))
    return {
        ...
        "target": stored["target"],
        "window": stored["window"],
        "headline_accuracy": stored["headline_accuracy"],
...
#
# Computed live: the queue is a handful of aggregate queries plus one pace run,
# and a stored snapshot would go stale the moment the as-of date moved -- which
# for an alert queue is the whole point.
# ---------------------------------------------------------------------------
from staypulse.analytics import alerts as _al  # noqa: E402


def rm_alert_center(as_of: dt.date) -> dict[str, Any]:
    return _al.alert_center(as_of)


def rm_opportunity_radar(as_of: dt.date) -> dict[str, Any]:
    return _al.opportunity_radar(as_of)


def rm_alert_summary(as_of: dt.date) -> dict[str, Any]:
    return _al.summary(as_of)


# ---------------------------------------------------------------------------
# Risk: cancellation model and overbooking simulator.
#
# The cancellation model is served from the stored evaluation -- fitting it is
# seconds of gradient descent and the answer does not change between requests.
# The overbooking simulator runs live, because its whole purpose is answering
# "what about THIS date at THIS cost ratio", which a snapshot cannot do.
# ---------------------------------------------------------------------------
from staypulse.analytics import overbooking as _ob  # noqa: E402

_RISK_EVAL = Path(__file__).resolve().parents[2] / "reports" / "risk.json"


def _risk_stored() -> dict[str, Any]:
    if not _RISK_EVAL.exists():
        raise RuntimeError("Risk evaluation has not been generated yet.")
    return json.loads(_RISK_EVAL.read_text(encoding="utf-8"))


def rm_cancellation_model() -> dict[str, Any]:
    stored = _risk_stored()["cancellation"]
    stored["source"] = "stored evaluation, regenerated by scripts/run_risk_analysis.py"
    return stored


def rm_overbooking(as_of: dt.date, stay_date: dt.date,
                   cost_ratio: float | None) -> dict[str, Any]:
    """Outcome table for one stay date; a recommendation only if priced.

    `cost_ratio` is passed straight through. There is no default and none is
    invented here: this warehouse prices neither a walked guest nor an empty
    room, so a recommendation without a supplied ratio would be fabricated.
    """
    return _ob.simulate_stay_date(as_of, stay_date, cost_ratio=cost_ratio)


def rm_overbooking_default(cost_ratio: float | None) -> dict[str, Any]:
    """Simulate the date where overbooking is actually a live decision.

    Every other endpoint here defaults its as-of date to something inside the
    data rather than failing, for the reason at the top of the router. The same
    applies to a stay date: refusing to answer without one makes the endpoint
    look broken when it is merely unparameterised.
    """
    tightest = _ob.tightest_stay_date()
    return _ob.simulate_stay_date(
...
_MODEL_REGISTRY = Path(__file__).resolve().parents[2] / "reports" / "model_registry.json"


def model_registry() -> dict[str, Any]:
    """Model cards and drift, from the stored evaluation."""
    if not _MODEL_REGISTRY.exists():
        raise RuntimeError("Model registry has not been generated yet.")
    stored = json.loads(_MODEL_REGISTRY.read_text(encoding="utf-8"))
    stored["source"] = "stored evaluation, regenerated by scripts/run_model_registry.py"
    return stored
